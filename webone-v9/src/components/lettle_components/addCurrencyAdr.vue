<template>
  <div class="addCurrencyAdr">
    <div class="title-user-profile green">绑定数字货币地址</div>
    <div class="from-box">
      <div>
        <span>数字货币地址</span>
        <p>
          <input type="text" placeholder="数字货币地址" 
                 v-model="currencyAdr" class="form-control" />
        </p>
      </div>
      <div>
        <span>备注</span>
      ...
      <div>
        ...
        <p class="addCurrencyAdr-yz">
          ...
          <label for="we11" v-show="EMAIL" style="cursor: pointer;">邮箱验证</label>
          <input name="we1" type="radio" value="mobiles" id="we12" v-show="MOBILE"/>
          <label for="we12" v-show="MOBILE" style="cursor: pointer;">手机验证</label>
        </p>
      </div>
      <div>
        <span>资金密码</span>
