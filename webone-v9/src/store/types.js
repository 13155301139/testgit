/**
 * Created by User on 2017/7/17.
 */
export const HEADER_SHOW = "HEADER_SHOW";
export const HEADER_HIDE = "HEADER_HIDE";
export const FOOTER_SHOW = "FOOTER_SHOW";
export const FOOTER_HIDE = "FOOTER_HIDE";

export const LOGINSTATE_TRUE = "LOGINSTATE_TRUE"; //登录
export const LOGINSTATE_FALSE = "LOGINSTATE_FALSE";//未登录

export const ERROR_SHOW='ERROR_SHOW';//错误处理
export const ERROR_HIDE='ERROR_HIDE';//错误处理
export const ERROR_SHOW1='ERROR_SHOW1';//错误处理
export const ERROR_HIDE1='ERROR_HIDE1';//错误处理


export const SING_LOG_SHOW='SING_LOG_SHOW';//签到日志显示
export const SING_LOG_HIDE='SING_LOG_HIDE';//签到日志隐藏

export const SETBUSINESSPRICENAME = 'SETBUSINESSPRICENAME';

export const SETBUSINESSPRICEDATA = 'SETBUSINESSPRICEDATA';

export const SETDEPTH = 'SETDEPTH';

export const SETTRADES = 'SETTRADES';

export const SETWEITUO = 'SETWEITUO';

export const SETCAPITALINFO = 'SETCAPITALINFO';


